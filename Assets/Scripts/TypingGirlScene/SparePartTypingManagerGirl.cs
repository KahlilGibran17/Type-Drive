...
public class SparePartTypingManagerGirl : MonoBehaviour
{
 ...
 void GenerateSentence()
{
    ...
    void SpawnSparePart()
    {
        ...
        if (SparePartPrefab != null)
        {
            ...
            PlayerPrefs.SetInt("SparePartTypeGirl",SparePartType);
            PlayerPrefs.Save();
...
void CheckInput()
{
    if (currentIndex < targetSentence.Length)
    {
                ...
                else
        {
            ...
            typo++;
            SFXGirl.Instance.PlayWrongSound();
            WrongLetterTyped();
            ...
            if (currentIndex == targetSentence.Length -1 )
            {
                ...
                SFXGirl.Instance.PlayCongratsSound();
 ...
 void GameOver()
    {
      int currentInstanceCount = PlayerPrefs.GetInt("CurrentInstanceCountGirl", 0);
        int currentInstanceIndex = currentInstanceCount + 1;

        PlayerPrefs.SetInt("ScoreGirl" + currentInstanceIndex, score);
        PlayerPrefs.SetInt("CustomerGirl" + currentInstanceIndex, 0);
        int sparePartType = PlayerPrefs.GetInt("SparePartTypeGirl",-1); // Example customer type for Girl
        PlayerPrefs.SetInt("SparePartTypeGirl" + currentInstanceIndex, sparePartType);
        PlayerPrefs.Save();

        PlayerPrefs.SetInt("CurrentInstanceCountGirl", currentInstanceCount + 1);
    ...
    void OnEnable()
    {
        PlayerPrefs.SetInt("HasAccessedSparePartGirlScene", 1);
    }
