...
public class SparePartTypingManagerThief : MonoBehaviour
{
 ...
 void SpawnSparePart()
    {
        ...
        if (SparePartPrefab != null)
        {
            ...
            PlayerPrefs.SetInt("SparePartTypeThief",SparePartType);
            PlayerPrefs.Save();
...
void CheckInput()
{
    if (currentIndex < targetSentence.Length)
    {
                ...
                else
        {
            ...
            typo++;
            SFXThief.Instance.PlayWrongSound();
            WrongLetterTyped();
            ...
            if (currentIndex == targetSentence.Length -1 )
            {
                ...
                SFXThief.Instance.PlayCongratsSound();
...
void GameOver()
    {
    int currentInstanceCount = PlayerPrefs.GetInt("CurrentInstanceCountThief", 0);
        int currentInstanceIndex = currentInstanceCount + 1;

        PlayerPrefs.SetInt("ScoreThief" + currentInstanceIndex, score - 10);
        PlayerPrefs.SetInt("CustomerThief" + currentInstanceIndex, 2);
        int sparePartType = PlayerPrefs.GetInt("SparePartTypeThief",-1); 
        PlayerPrefs.SetInt("SparePartTypeThief" + currentInstanceIndex, sparePartType);
        PlayerPrefs.Save();

        PlayerPrefs.SetInt("CurrentInstanceCountThief", currentInstanceCount + 1);
...
void OnEnable()
{
    PlayerPrefs.SetInt("HasAccessedSparePartThiefScene", 1);
}
